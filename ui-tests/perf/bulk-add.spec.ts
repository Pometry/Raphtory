...
test.describe('bulk-add perf', () => {
    test('expand CMP-000 (~2.7k neighbours) and time to layout-stable', async ({
        ...
        for (let i = 0; i < 5; i++) {
            ...
            await page.waitForFunction(
                () =>
                    window.__PERF__
                        ?.dump()
                        .some((s) => s.operation === 'bulk-add' && s.endedAt > s.startedAt) ??
                    false,
                undefined,
            ...
            if (sample !== undefined && sample.metadata?.nodeCount !== undefined) {
                samples.push({
        ...
        const sortedDur = [...warmedUp].map((s) => s.durationMs).sort((a, b) => a - b);
        const median = sortedDur[Math.floor(sortedDur.length / 2)] ?? 0;
        const p95Index = Math.min(Math.floor(sortedDur.length * 0.95), sortedDur.length - 1);
        ...
        console.log(`[bulk-add perf] median ${median.toFixed(0)} ms, p95 ${p95.toFixed(0)} ms`);
