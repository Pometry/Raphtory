...
test('numerical graph renders integer-ID nodes, including u64 max', async ({
    page,
}) => {
    ...
    expect(ids).toEqual(
        new Set(['1', '2', '3', '4', MAX_U64, MAX_U64_MINUS_1]),
    );
});

test('can render graph with numerical ID node', async ({ page }) => {
    await page.goto(
        `/graph/vanilla/numerical?initialNodes=%5B%22${MAX_U64}%22%5D`,
    );
    await waitForLayoutToFinish(page);
