...
def bench_polars(df: pl.DataFrame) -> float:
    g = Graph()
    start = time.perf_counter()
...
def bench_arrow(df: pl.DataFrame) -> float:
    g = Graph()
...
def ingestion_speed_btc_dataset():
    ...
    for _ in range(5):
        ...
        # 3.1) Polars ingestion (to_pandas() called internally)
        polars_time = bench_polars(df=df_pl)
        polars_ingestion_times.append(polars_time)
        gc.collect()

        # 3.2) Polars ingestion streaming (no internal to_pandas() call)
        polars_streaming_time = bench_polars_streaming(df=df_pl)
        polars_streaming_ingestion_times.append(polars_streaming_time)
        gc.collect()

        # 4.1) Arrow ingestion
        arrow_time = bench_arrow(df_pl)
        arrow_ingestion_times.append(arrow_time)
        gc.collect()

        # 4.2) Arrow ingestion streaming
        arrow_streaming_time = bench_arrow_streaming(df_pl)
        arrow_streaming_ingestion_times.append(arrow_streaming_time)
        gc.collect()

        # 5.1) DuckDB ingestion (fetch_arrow_table() called internally)
        duckdb_time = bench_duckdb(df_pl)
        duckdb_ingestion_times.append(duckdb_time)
        gc.collect()

        # 5.2) DuckDB streaming ingestion (no internal fetch_arrow_table() call)
