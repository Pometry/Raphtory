...
use self::fields::{NAME, SRC_ID, TIME, VERTEX_ID, VERTEX_ID_REV};
...
impl<G: GraphViewOps> IndexedGraph<G> {
    ...
    fn new_edge_schema_builder() -> SchemaBuilder {
        ...
        schema.add_i64_field(fields::TIME, INDEXED | STORED);
        // ensure we add vertex_id as stored to get back the vertex id after the search
        schema.add_u64_field(fields::SRC_ID, FAST | STORED);
        schema.add_u64_field(fields::SOURCE, INDEXED);

        schema.add_u64_field(fields::DEST_ID, FAST | STORED);
        schema.add_u64_field(fields::DESTINATION, INDEXED);
    ...
    fn vertex_ref_to_id(v_ref: VertexRef) -> u64 {
        match v_ref {
            VertexRef::Local(v_id) => Into::<usize>::into(v_id) as u64,
            VertexRef::Remote(v_id) => panic!("remote vertex id {} not supported yet", v_id),
        }
    }
    ...
    fn index_edge_view<W: Deref<Target = IndexWriter>>(
        ...
        for temp_prop_name in temp_prop_names {
            ...
            for (time, prop_value) in e_ref.property_history(&temp_prop_name) {
                ...
                document.add_i64(time_field, time);
                // add the vertex_id
                document.add_u64(src_id_field, Self::vertex_ref_to_id(edge_ref.src()));
                document.add_u64(dst_id_field, Self::vertex_ref_to_id(edge_ref.dst()));
    ...
    pub fn from_graph_edges(g: &G) -> tantivy::Result<Self> {
        let schema = Self::schema_for_edge(g);
        let (index, reader) = Self::new_index(schema.clone(), Self::default_edge_index_settings());

        let time_field = schema.get_field(fields::TIME)?;
        let src_id_field = schema.get_field(fields::SRC_ID)?;
        let dst_id_field = schema.get_field(fields::DEST_ID)?;
