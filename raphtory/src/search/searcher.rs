use crate::{
    ...
    db::{
        api::{
            ...
            view::{
                internal::{core_ops::CoreGraphOps, InternalIndexSearch},
                StaticGraphViewOps,
            },
        },
...
impl<'a> Searcher<'a> {
    ...
    pub fn search_nodes<G: StaticGraphViewOps>(
        ...
        let query: Box<dyn Query> = query_parser.parse_query(q)?;
    ...
    // search node by node_type, node_name => custom query => search node indexes
    // search node by prop_name:prop_value => custom query => search prop indexes
