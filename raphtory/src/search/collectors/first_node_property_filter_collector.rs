...
pub struct FirstNodePropertyFilterCollector<G> {
    prop_id: usize,
    field: String,
    graph: G,
}

impl<G> FirstNodePropertyFilterCollector<G>
where
...
impl<G> Collector for FirstNodePropertyFilterCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<u64>;
    type Child = FirstNodePropertyFilterSegmentCollector<G>;
    ...
    fn for_segment(
        ...
        Ok(FirstNodePropertyFilterSegmentCollector {
...
pub struct FirstNodePropertyFilterSegmentCollector<G> {
    prop_id: usize,
...
impl<G> SegmentCollector for FirstNodePropertyFilterSegmentCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<u64>;
    ...
    fn collect(&mut self, doc_id: u32, _score: Score) {
            ...
            (opt_time, opt_entity_id, opt_secondary_time)
        {
            if !self.unique_entity_ids.contains(&entity_id) {
                if self.graph.is_node_prop_update_first(
                    self.prop_id,
