...
pub struct LatestExplodedEdgePropertyFilterCollector<G> {
    prop_id: usize,
    field: String,
    graph: G,
}

impl<G> LatestExplodedEdgePropertyFilterCollector<G>
where
...
impl<G> Collector for LatestExplodedEdgePropertyFilterCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<(TimeIndexEntry, EID, usize)>;
    type Child = LatestExplodedEdgePropertyFilterSegmentCollector<G>;
    ...
    fn for_segment(
        ...
        Ok(LatestExplodedEdgePropertyFilterSegmentCollector {
    ...
    fn merge_fruits(
        &self,
        segment_fruits: Vec<HashSet<(TimeIndexEntry, EID, usize)>>,
    ) -> tantivy::Result<HashSet<(TimeIndexEntry, EID, usize)>> {
        let mut global_unique_entity_ids: HashSet<(TimeIndexEntry, EID, usize)> = HashSet::new();
...
pub struct LatestExplodedEdgePropertyFilterSegmentCollector<G> {
    prop_id: usize,
...
impl<G> SegmentCollector for LatestExplodedEdgePropertyFilterSegmentCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<(TimeIndexEntry, EID, usize)>;
    ...
    fn collect(&mut self, doc_id: u32, _score: Score) {
        ...
        {
            let tie = TimeIndexEntry::new(time, secondary_time as usize);
            ...
            if !self.unique_entity_ids.contains(&(tie, eid, layer_id))
                && self.graph.is_edge_prop_update_latest(
