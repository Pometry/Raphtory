...
pub struct FirstExplodedEdgePropertyFilterCollector<G> {
    prop_id: usize,
    field: String,
    graph: G,
}

impl<G> FirstExplodedEdgePropertyFilterCollector<G>
where
...
impl<G> Collector for FirstExplodedEdgePropertyFilterCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<(TimeIndexEntry, EID, usize)>;
    type Child = FirstExplodedEdgePropertyFilterSegmentCollector<G>;
    ...
    fn for_segment(
        ...
        Ok(FirstExplodedEdgePropertyFilterSegmentCollector {
...
pub struct FirstExplodedEdgePropertyFilterSegmentCollector<G> {
    prop_id: usize,
...
impl<G> SegmentCollector for FirstExplodedEdgePropertyFilterSegmentCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<(TimeIndexEntry, EID, usize)>;
    ...
    fn collect(&mut self, doc_id: u32, _score: Score) {
            ...
            (opt_time, opt_entity_id, opt_layer_id, opt_secondary_time)
        {
            ...
            if !self.unique_entity_ids.contains(&(tie, eid, layer_id))
                && self.graph.is_edge_prop_update_first(
