...
pub struct FirstEdgePropertyFilterCollector<G> {
    prop_id: usize,
    field: String,
    graph: G,
}

impl<G> FirstEdgePropertyFilterCollector<G>
where
...
impl<G> Collector for FirstEdgePropertyFilterCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<u64>;
    type Child = FirstEdgePropertyFilterSegmentCollector<G>;
    ...
    fn for_segment(
        ...
        Ok(FirstEdgePropertyFilterSegmentCollector {
...
pub struct FirstEdgePropertyFilterSegmentCollector<G> {
    prop_id: usize,
...
impl<G> SegmentCollector for FirstEdgePropertyFilterSegmentCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<u64>;
    ...
    fn collect(&mut self, doc_id: u32, _score: Score) {
            ...
            (opt_time, opt_entity_id, opt_layer_id, opt_secondary_time)
        {
            ...
            if !self.unique_entity_ids.contains(&entity_id) {
                if self.graph.is_edge_prop_update_first(
