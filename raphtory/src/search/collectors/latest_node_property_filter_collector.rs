...
impl<G> LatestNodePropertyFilterCollector<G>
where
...
impl<G> Collector for LatestNodePropertyFilterCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<u64>;
    type Child = LatestNodePropertyFilterSegmentCollector<G>;
    ...
    fn for_segment(
        ...
        Ok(LatestNodePropertyFilterSegmentCollector {
...
pub struct LatestNodePropertyFilterSegmentCollector<G> {
    prop_id: usize,
...
impl<G> SegmentCollector for LatestNodePropertyFilterSegmentCollector<G>
where
    G: StaticGraphViewOps,
{
    type Fruit = HashSet<u64>;
    ...
    fn collect(&mut self, doc_id: u32, _score: Score) {
        ...
        if let (Some(time), Some(entity_id), Some(event_id)) =
            (opt_time, opt_entity_id, opt_event_id)
        {
