...
pub struct ResultUtcDateTimeIterable {
    builder: Arc<dyn Fn() -> BoxedIter<Result<DateTime<Utc>, TimeError>> + Send + Sync + 'static>,
}

impl Repr for ResultUtcDateTimeIterable {
    fn repr(&self) -> String {
        ...
        format!("ResultUtcDateTimeIterable([{}])", content)
    }
}

impl ResultUtcDateTimeIterable {
    pub fn iter(&self) -> BoxedIter<Result<DateTime<Utc>, TimeError>> {
        (self.builder)()
    }
}

impl<F: Fn() -> It + Send + Sync + 'static, It: Iterator + Send + Sync + 'static> From<F>
    for ResultUtcDateTimeIterable
where
    It::Item: Into<Result<DateTime<Utc>, TimeError>>,
{
    fn from(value: F) -> Self {
        let builder = Arc::new(move || {
            let iter: BoxedIter<Result<DateTime<Utc>, TimeError>> =
...
#[pyclass(frozen, module = "raphtory")]
pub struct NestedResultUtcDateTimeIterable {
    builder: Arc<
        dyn Fn() -> BoxedIter<BoxedIter<Result<DateTime<Utc>, TimeError>>> + Send + Sync + 'static,
    >,
}

impl Repr for NestedResultUtcDateTimeIterable {
    fn repr(&self) -> String {
        ...
        format!("NestedResultUtcDateTimeIterable([{}])", content)
    }
}

impl NestedResultUtcDateTimeIterable {
    pub fn iter(&self) -> BoxedIter<BoxedIter<Result<DateTime<Utc>, TimeError>>> {
        (self.builder)()
    }
}

impl<F: Fn() -> It + Send + Sync + 'static, It: Iterator + Send + Sync + 'static> From<F>
    for NestedResultUtcDateTimeIterable
where
    It::Item: Iterator + Send + Sync,
    <It::Item as Iterator>::Item: Into<Result<DateTime<Utc>, TimeError>> + Send + Sync,
{
    fn from(value: F) -> Self {
        let builder = Arc::new(move || {
            let iter: BoxedIter<BoxedIter<Result<DateTime<Utc>, TimeError>>> =
                Box::new(value().map(|inner_it| {
                    let inner_iter: BoxedIter<Result<DateTime<Utc>, TimeError>> =
