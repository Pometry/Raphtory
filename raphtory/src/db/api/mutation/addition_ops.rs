...
impl<G: InternalAdditionOps<Error: Into<GraphError>> + StaticGraphViewOps> AdditionOps for G {
    ...
    fn flush(&self) -> Result<(), Self::Error> {
        ...
        {
            use crate::{
                db::api::view::internal::InternalStorageOps,
                serialise::metadata::refresh_disk_graph_metadata,
            };

            if let Some(disk_path) = self.disk_storage_path() {
                let disk_path = disk_path.to_path_buf();
                refresh_disk_graph_metadata(&disk_path, self.count_nodes(), self.count_edges())
                    .map_err(|err| MutationError::from(err).into())?;
            }
        }
