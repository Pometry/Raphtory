...
macro_rules! for_all {
    ($value:expr, $pattern:pat => $result:expr) => {
        match $value {
            DirectionVariants::Out($pattern) => $result,
            DirectionVariants::In($pattern) => $result,
            DirectionVariants::Both($pattern) => $result,
        }
    };
}
...
impl<V, Out: Iterator<Item = V>, In: Iterator<Item = V>, Both: Iterator<Item = V>> Iterator
    for DirectionVariants<Out, In, Both>
{
    type Item = V;
...
impl<
        V,
        Out: DoubleEndedIterator<Item = V>,
        In: DoubleEndedIterator<Item = V>,
        Both: DoubleEndedIterator<Item = V>,
    > DoubleEndedIterator for DirectionVariants<Out, In, Both>
{
...
impl<
        V,
        Out: ExactSizeIterator<Item = V>,
        In: ExactSizeIterator<Item = V>,
        Both: ExactSizeIterator<Item = V>,
    > ExactSizeIterator for DirectionVariants<Out, In, Both>
{
    fn len(&self) -> usize {
...
impl<
        V: Send,
        Out: ParallelIterator<Item = V>,
        In: ParallelIterator<Item = V>,
        Both: ParallelIterator<Item = V>,
    > ParallelIterator for DirectionVariants<Out, In, Both>
{
    type Item = V;
...
impl<
        V: Send,
        Out: IndexedParallelIterator<Item = V>,
        In: IndexedParallelIterator<Item = V>,
        Both: IndexedParallelIterator<Item = V>,
    > IndexedParallelIterator for DirectionVariants<Out, In, Both>
{
    fn len(&self) -> usize {
        for_all!(self, iter => iter.len())
    }
