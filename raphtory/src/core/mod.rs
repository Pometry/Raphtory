...
// define static table
// Str -> not list, map?, array, (possible parse if separator)
// numerical type -> range check, not list, map, array (possibly wrap in list or array)
// bool -> not collection (possibly wrap in list or array)
// list -> array (map if tuples), unwrap if single element
// array -> list (map if tuples), unwrap if single element
// dtime -> not collections or bool (possibly wrap), check range,
// map -> list, array, nothing else
...
impl Prop {
    ...
    fn try_into_int<T: TryFromInt>(self) -> Result<T, GraphError> {
        match self {
            Prop::U8(v) => TryInto::<T>::try_into(v).map_err(|_| GraphError::ParsingError),
            Prop::U16(v) => TryInto::<T>::try_into(v).map_err(|_| GraphError::ParsingError),
            Prop::I32(v) => TryInto::<T>::try_into(v).map_err(|_| GraphError::ParsingError),
            Prop::I64(v) => TryInto::<T>::try_into(v).map_err(|_| GraphError::ParsingError),
            Prop::U32(v) => TryInto::<T>::try_into(v).map_err(|_| GraphError::ParsingError),
            Prop::U64(v) => TryInto::<T>::try_into(v).map_err(|_| GraphError::ParsingError),
            Prop::F32(v) => Prop::float_to_int(v)
                .map_err(|_| GraphError::ParsingError)?
                .try_into()
                .map_err(|_| GraphError::ParsingError),
    ...
    fn into_f64(self) -> Result<f64, GraphError> {
        match self {
    ...
    fn try_into_f32<T: Into<f64> + Copy>(value: T) -> Result<Prop, GraphError> {
        let as_f64 = value.into();
        if as_f64 >= f32::MIN as f64 && as_f64 <= f32::MAX as f64 {
            Ok(Prop::F32(as_f64 as f32))
        } else {
            Err(GraphError::ParsingError)
        }
    }

    pub fn try_cast(self, prop_type: PropType) -> Result<Prop, GraphError> {
        match self {
