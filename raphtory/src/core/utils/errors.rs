...
pub enum GraphError {
    ...
    #[error("The layer_name function is only available once an edge has been exploded via .explode_layers(). If you want to retrieve the layers for this edge you can use .layer_names")]
    LayerNameAPIError,

    #[error("The time function is only available once an edge has been exploded via .explode_layers() and .explode(). You may want to retrieve the history for this edge via .history(), or the earliest/latest time via earliest_time or latest_time")]
    TimeAPIError,
}
