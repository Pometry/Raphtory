...
impl TPropColumn {
    ...
    pub(crate) fn set(&mut self, index: usize, prop: Prop) -> Result<(), GraphError> {
        ...
        match (self, prop) {
            ...
            (TPropColumn::Blob(col), Prop::Array(v)) => col.set(index, v)?,
    ...
    pub(crate) fn push(&mut self, prop: Prop) -> Result<(), GraphError> {
        ...
        match (self, prop) {
            ...
            (TPropColumn::Blob(col), Prop::Array(v)) => col.push(Some(v)),
    ...
    fn init_empty_col(&mut self, prop: &Prop) -> Result<(), GraphError> {
        match self {
            TPropColumn::Empty(len) => match prop {
                ...
                Prop::Array(_) => *self = TPropColumn::Blob(LazyVec::with_len(*len)),
    ...
    pub(crate) fn push_null(&mut self) {
        match self {
            ...
            TPropColumn::Blob(col) => col.push(None),
    ...
    pub(crate) fn get(&self, index: usize) -> Option<Prop> {
        match self {
            ...
            TPropColumn::Blob(col) => col.get_opt(index).map(|prop| Prop::Array(prop.clone())),
