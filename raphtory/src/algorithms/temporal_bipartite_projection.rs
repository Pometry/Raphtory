...
use crate::{
    ...
    prelude::{EdgeListOps, EdgeViewOps, GraphViewOps, VertexViewOps, NO_PROPS, PropUnwrap}, db::api::view::internal::{GraphOps, DynamicGraph},
};
...
pub fn temporal_bipartite_projection<G:GraphViewOps, >(graph:&G, delta:i64, pivot_type:String) -> Graph {
    let new_graph = Graph::new();
    let nodes = graph.vertices().iter().filter(|v| v.properties().get("Type").unwrap_str()==pivot_type);
    for v in nodes {populate_edges(graph, &new_graph, v, delta)};
    new_graph
}
    ...
    if let Some(vertex) = g.vertex(v) {
        ...
        for nb in visitors.iter() {
            ...
            for node in &to_process {
                ...
                new_graph.add_edge(new_time, node.name.clone(), nb.name.clone(), NO_PROPS, None).unwrap();
            }
...
mod bipartite_graph_tests {
    ...
    use crate::{
        ...
        prelude::{NO_PROPS, Prop},
    };

    #[test]
    fn small_delta_test() {
        let g = Graph::new();
        let vs = vec![(1,"A","1"), (3,"A","2"), (3,"B","2"), (4,"C","3"),(6,"B","3"),(8,"A","3"),(10,"C","4"),(11,"B","4")];
        for (t, src, dst) in &vs {
            g.add_vertex(*t, *src, [("Type",Prop::Str("Left".into()))]).unwrap();
            g.add_vertex(*t, *dst, [("Type",Prop::Str("Right".into()))]).unwrap();
