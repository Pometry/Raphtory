    ...
    G: StaticGraphViewOps,
{
    ...
    let step2 = ATask::new(move |vv: &mut EvalNodeView<_, FastRPState>| {
        ...
        let weights = Arc::clone(&weights);
        let denom: f64 = weights[vv.graph().ss.clone()]
            / ((vv.neighbours().iter().count() * (num_iters + 1)) as f64);
...
mod fast_rp_test {
    ...
    use super::*;
    use crate::{
        core::entities::nodes::node_ref::AsNodeRef, db::api::mutation::AdditionOps, prelude::*,
        test_storage,
    };
