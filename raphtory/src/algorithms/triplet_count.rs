...
impl Program for TripletCount {
    ...
    fn local_eval<G: GraphViewOps>(&self, c: &LocalState<G>) {
        let count = c.global_agg(state::accumulator_id::def::sum::<usize>(111));
    ...
    fn post_eval<G: GraphViewOps>(&self, c: &mut GlobalEvalState<G>) {
        let _ = c.global_agg(state::accumulator_id::def::sum::<usize>(111));
        c.step(|_| false)
    }
    ...
    where
        Self: Sync,
    {
        gs.read_global_state(&state::accumulator_id::def::sum::<usize>(111))
            .unwrap_or(0)
    }
}
