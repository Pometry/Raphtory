...
mod page_rank_tests {
    ...
    fn test_page_rank() {
        ...
        let results: AlgorithmResult<OrderedFloat<f64>> = unweighted_page_rank(&graph, 1000, Some(1), None, true);

        assert_eq_f64(Some(&results.get("1").unwrap().0), Some(&0.38694), 5);
        assert_eq_f64(Some(&results.get("2").unwrap().0), Some(&0.20195), 5);
        assert_eq_f64(Some(&results.get("4").unwrap().0), Some(&0.20195), 5);
        assert_eq_f64(Some(&results.get("3").unwrap().0), Some(&0.20916), 5);
    }
    ...
    fn motif_page_rank() {
        ...
        let results: AlgorithmResult<OrderedFloat<f64>> = unweighted_page_rank(&graph, 1000, Some(4), None, true);

        assert_eq_f64(Some(&results.get("10").unwrap().0), Some(&0.072082), 5);
        assert_eq_f64(Some(&results.get("8").unwrap().0), Some(&0.136473), 5);
        assert_eq_f64(Some(&results.get("3").unwrap().0), Some(&0.15484), 5);
        assert_eq_f64(Some(&results.get("6").unwrap().0), Some(&0.07208), 5);
        assert_eq_f64(Some(&results.get("11").unwrap().0), Some(&0.06186), 5);
        assert_eq_f64(Some(&results.get("2").unwrap().0), Some(&0.03557), 5);
        assert_eq_f64(Some(&results.get("1").unwrap().0), Some(&0.11284), 5);
        assert_eq_f64(Some(&results.get("4").unwrap().0), Some(&0.07944), 5);
        assert_eq_f64(Some(&results.get("7").unwrap().0), Some(&0.01638), 5);
        assert_eq_f64(Some(&results.get("9").unwrap().0), Some(&0.06186), 5);
        assert_eq_f64(Some(&results.get("5").unwrap().0), Some(&0.19658), 5);
    }
    ...
    fn two_nodes_page_rank() {
        ...
        let results: AlgorithmResult<OrderedFloat<f64>> =
            unweighted_page_rank(&graph, 1000, Some(4), None, false);

        assert_eq_f64(Some(&results.get("1").unwrap().0), Some(&0.5), 3);
        assert_eq_f64(Some(&results.get("2").unwrap().0), Some(&0.5), 3);
    }
    ...
    fn three_nodes_page_rank_one_dangling() {
        ...
        let results: AlgorithmResult<OrderedFloat<f64>> = unweighted_page_rank(&graph, 10, Some(4), None, false);

        assert_eq_f64(Some(&results.get("1").unwrap().0), Some(&0.303), 3);
        assert_eq_f64(Some(&results.get("2").unwrap().0), Some(&0.393), 3);
        assert_eq_f64(Some(&results.get("3").unwrap().0), Some(&0.303), 3);
    }
    ...
    fn dangling_page_rank() {
        ...
        assert_eq_f64(Some(&results.get("1").unwrap().0), Some(&0.055), 3);
        assert_eq_f64(Some(&results.get("2").unwrap().0), Some(&0.079), 3);
        assert_eq_f64(Some(&results.get("3").unwrap().0), Some(&0.113), 3);
        assert_eq_f64(Some(&results.get("4").unwrap().0), Some(&0.055), 3);
        assert_eq_f64(Some(&results.get("5").unwrap().0), Some(&0.070), 3);
        assert_eq_f64(Some(&results.get("6").unwrap().0), Some(&0.083), 3);
        assert_eq_f64(Some(&results.get("7").unwrap().0), Some(&0.093), 3);
        assert_eq_f64(Some(&results.get("8").unwrap().0), Some(&0.102), 3);
        assert_eq_f64(Some(&results.get("9").unwrap().0), Some(&0.110), 3);
        assert_eq_f64(Some(&results.get("10").unwrap().0), Some(&0.117), 3);
        assert_eq_f64(Some(&results.get("11").unwrap().0), Some(&0.122), 3);
    }
