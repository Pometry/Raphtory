...
pub async fn prepare_plan(
    ...
    for layer in graph.unique_layers() {
        let edge_list_table = EdgeListTableProvider::new(layer.as_ref(), g.clone())?;
        ctx.register_table(layer.as_ref(), Arc::new(edge_list_table))?;
    }

    // let node_table_provider = NodeTableProvider::new(g.clone())?;
    // ctx.register_table("nodes", Arc::new(node_table_provider))?;
    let layer_names = graph.unique_layers().map(String::from).collect::<Vec<_>>();
...
pub async fn run_sql(query: &str, graph: &Graph) -> Result<DataFrame, ExecError> {
    let ctx = SessionContext::new();

    for layer in graph.unique_layers() {
        let table = EdgeListTableProvider::new(layer.as_ref(), graph.clone())?;
        ctx.register_table(layer.as_ref(), Arc::new(table))?;
    }
