...
pub fn bootstrap_graph(num_vertices: usize) -> Graph {
    ...
    for _ in 0..num_edges {
        ...
        graph.add_edge(time, source, target, EMPTY, None).unwrap();
    }
    graph
}
    ...
    F: FnMut() -> Graph,
{
    ...
    bench(
        ...
        |b: &mut Bencher| {
            b.iter_batched_ref(
                || (make_graph(), time_sample()),
                |(g, t)| g.add_edge(*t, 0, 0, EMPTY, None),
                BatchSize::SmallInput,
            )
        },
    );
    bench(
        ...
        |b: &mut Bencher| {
            b.iter_batched_ref(
                ...
                |(g, s, d)| g.add_edge(0, *s, *d, EMPTY, None),
                BatchSize::SmallInput,
            )
        },
    );
    ...
    bench(
        ...
        |b: &mut Bencher| {
            b.iter_batched_ref(
                ...
                |(g, times)| {
                    for t in times.iter() {
                        g.add_edge(*t, 0, 0, EMPTY, None).unwrap()
                    }
                },
                BatchSize::SmallInput,
            )
    ...
    bench(
        ...
        |b: &mut Bencher| {
            b.iter_batched_ref(
                ...
                |(g, times)| {
                    for t in times.iter() {
                        g.add_edge(*t, "test", "other", EMPTY, None).unwrap()
                    }
                },
                BatchSize::SmallInput,
            )
