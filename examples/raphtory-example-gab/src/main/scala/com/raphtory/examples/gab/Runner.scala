...
import com.raphtory.core.components.spout.instance.{FileSpout, ResourceSpout}
import com.raphtory.output.PulsarOutputFormat

object Runner extends App{
        val source: Spout[String]    = ResourceSpout("gabNetwork500.csv")
        val builder   = new GabUserGraphBuilder()
        val rg        = Raphtory.createGraph(spout = source,graphBuilder = builder)
        Thread.sleep(60000)
        val outputFormat = PulsarOutputFormat("Gab")
        rg.pointQuery(EdgeList(), PulsarOutputFormat("EdgeList"), timestamp=1476113868000L)
        rg.rangeQuery(ConnectedComponents(), outputFormat = outputFormat, start = 1470797917000L, end = 1476113868000L, increment = 86400000L, windows = List(3600000L, 86400000L, 604800000L, 2592000000L, 31536000000L))
