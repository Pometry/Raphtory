    ...
    let graph = if encoded_data_dir.exists() {
        ...
        CsvLoader::new(data_dir)
            ...
            .load_into_graph(&g, |stablecoin: StableCoin, g: &Graph| {
                let src_id = utils::calculate_hash(&stablecoin.from_address);
                let dst_id = utils::calculate_hash(&stablecoin.to_address);
                let time = stablecoin.time_stamp;

                g.add_edge(time, src_id, dst_id, &vec![], None)
