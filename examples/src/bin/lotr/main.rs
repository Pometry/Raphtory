...
fn main() {
    ...
    // graph.vertices_window(0, i64::MAX, 100).for_each(|v| {
    //     println!("{:?}", v);
    // });
    ...
    // Test 1: All of time 
    g.vertices_window(3, 6, 2).for_each(|v| {
        println!("{:?}", v);
    });

    println!("next graph");

    let g2 = GraphDB::new(3);

    for (src, dst, t) in &vs {
        g2.add_edge(*src, *dst, *t, &vec![]);
    }
    // Test 1: All of time 
    g2.vertices_window(3, 6, 2).for_each(|v| {
        println!("{:?}", v);
    });
