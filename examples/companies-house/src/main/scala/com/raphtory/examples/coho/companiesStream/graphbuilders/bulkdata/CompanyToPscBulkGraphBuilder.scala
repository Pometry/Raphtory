...
class CompanyToPscBulkGraphBuilder extends GraphBuilder[String] {
  ...
  def sendPscToPartitions(psc: PersonWithSignificantControlStream, graph: Graph) = {
    ...
    val notifiedOn =
      LocalDate
        .parse(
                psc.data.get.notified_on.getOrElse("1800-01-01").replaceAll("\"", ""),
    ...
    if (notifiedOn > 0)
      if (psc.data.get.ceased_on.nonEmpty) {
        ...
        val ceasedOn = LocalDate
          .parse(psc.data.get.ceased_on.get.replaceAll("\"", ""), DateTimeFormatter.ofPattern("yyyy-MM-dd"))
          .toEpochSecond(LocalTime.MIDNIGHT, ZoneOffset.MIN) * 1000
        ...
        graph.addEdge(
                ...
                assignID(nameID),
                assignID(companyNumber),
                Properties(MutableInteger("weight", shareOwnership)),
