...
//    println("Parsed record:  msgId: " + msgId + " date: " + date + " sender: " + sender + " receiver: " + receiver)
    ...
    addVertex(dateEpochUTC, tarID, Properties(ImmutableProperty("name",receiver)),Type("Character"))
    addEdge(dateEpochUTC, srcID, tarID, Properties(ImmutableProperty("MsgId",msgId), ImmutableProperty("Subject",subject.get)), Type("Character"))
