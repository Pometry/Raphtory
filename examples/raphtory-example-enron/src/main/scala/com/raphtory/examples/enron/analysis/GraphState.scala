package com.raphtory.examples.enron.analysis
...
class GraphState() extends GraphAlgorithm{
  override def tabularise(graph: GraphPerspective): Table = {
    graph.select(vertex => {
      ...
      Row(vertex.ID(), inDeg, outDeg, degSum, vdeletions, vcreations, outedgedeletions, outedgecreations, inedgedeletions, inedgecreations, properties, propertyhistory, outedgeProperties, outedgePropertyHistory, inedgeProperties, inedgePropertyHistory)
    })
  }
}
