...
def test_graphql():
    ...
    map_server = RaphtoryServer(graphs=graphs).start_with_port(1737)
...
def test_graphqlclient():
    ...
    dir_server = RaphtoryServer(graph_dir=temp_dir).start_with_port(1737)
    raphtory_client = RaphtoryClient("http://localhost:1737")
    generic_client_test(raphtory_client, temp_dir)
    dir_server.stop()
    dir_server.wait()

    raphtory_client = RaphtoryServer(graph_dir=temp_dir).start_with_port(1737)
    generic_client_test(raphtory_client, temp_dir)
    raphtory_client.stop()
    raphtory_client.wait()

    raphtory_client = RaphtoryServer(graph_dir=temp_dir).start_with_port(1738)
    generic_client_test(raphtory_client, temp_dir)
    raphtory_client.stop()
    raphtory_client.wait()
