...
def test_temporal_constant(GraphClass):
    ...
    test_cases = [
        ...
        (filter.ExplodedEdge.property("weight").temporal().latest() == 2, 2),
        (filter.ExplodedEdge.property("weight").temporal().latest() != 3, 4),
        (filter.ExplodedEdge.property("weight").temporal().latest() < 3, 4),
        (filter.ExplodedEdge.property("weight").temporal().latest() > 1, 4),
        (filter.ExplodedEdge.property("weight").temporal().latest() <= 2, 4),
        (filter.ExplodedEdge.property("weight").temporal().latest() >= 3, 2),
        (filter.ExplodedEdge.property("weight").temporal().latest().is_in([1, 2]), 4),
