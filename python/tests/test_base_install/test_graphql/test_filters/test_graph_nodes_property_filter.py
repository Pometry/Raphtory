...
def test_graph_node_property_filter_less_than_or_equal_type_error(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop1", where: { le: { str: "shivam" } } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_graph_node_property_filter_greater_than(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop1", where: { gt: { i64: 30 } } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_graph_node_property_filter_greater_than_type_error(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop1", where: { gt: { str: "shivam" } } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_graph_node_property_filter_less_than(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop1", where: { lt: { i64: 30 } } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_graph_node_property_filter_less_than_type_error(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop1", where: { lt: { str: "shivam" } } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_graph_node_property_filter_is_none(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop5", where: { isNone: true } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_graph_node_property_filter_is_some(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop5", where: { isSome: true } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_graph_node_property_filter_is_in(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop1", where: { isIn: { list: [{i64: 10},{i64: 30},{i64: 50},{i64: 70}] } } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_graph_node_property_filter_is_in_type_error(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop1", where: { isIn: { str: "shivam" } } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_graph_node_property_filter_is_not_in_any(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop1", where: { isNotIn: { list: [{i64: 10},{i64: 30},{i64: 50},{i64: 70}] } } } }
        ) {
          nodes { list { name } }
        }
      }
...
def test_node_property_filter_not_is_not_in_empty_list(graph):
    ...
    query {
      graph(path: "g") {
        nodes {
          filter(
            expr: { property: { name: "prop1", where: { isNotIn: { list: [] } } } }
          ) {
            list { name }
          }
        }
...
def test_graph_node_property_filter_is_not_in_type_error(graph):
    ...
    query {
      graph(path: "g") {
        filterNodes(
          expr: { property: { name: "prop1", where: { isNotIn: { str: "shivam" } } } }
        ) {
          nodes { list { name } }
        }
      }
