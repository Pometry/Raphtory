...
def test_out_component_scoped_by_edge_layer(client):
    ...
    assert _names(
        ...
        "a",
        "outComponent",
        '{edges: {layers: {names: ["owns"], expr: {isValid: true}}}}',
    ) == ["b", "c"]
    ...
    assert _names(
        ...
        "a",
        "outComponent",
        '{edges: {layers: {names: ["has"], expr: {isValid: true}}}}',
    ) == ["x"]
...
def test_out_component_scoped_by_node_filter(client):
    ...
    assert _names(
        ...
        "a",
        "outComponent",
        '{nodes: {name: {where: {isIn: {list: [{str: "b"}, {str: "c"}]}}}}}',
    ) == ["b", "c"]
...
def test_component_filter_and_or_combinators(client):
    ...
    assert _names(
        ...
        "a",
        "outComponent",
        '{edges: {or: [{layers: {names: ["owns"], expr: {isValid: true}}}, '
    ...
    assert _names(
        ...
        "a",
        "outComponent",
        '{edges: {and: [{layers: {names: ["owns"], expr: {isValid: true}}}, {isValid: true}]}}',
    ) == ["b", "c"]
...
def test_in_component_scoped_by_filters(client):
    ...
    assert _names(
        ...
        "c",
        "inComponent",
        '{nodes: {name: {where: {isIn: {list: [{str: "a"}, {str: "b"}]}}}}}',
    ) == ["a", "b"]
    # edge filter
    assert _names(
        client,
