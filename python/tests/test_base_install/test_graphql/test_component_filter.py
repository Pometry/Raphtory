...
def test_component_filter_and_or_combinators():
    ...
    with _served().start() as server:
        ...
        assert _names(
            ...
            "a",
            "outComponent",
            '{edge: {or: [{layers: {names: ["owns"], expr: %s}}, {layers: {names: ["has"], expr: %s}}]}}'
            % (PASS_EDGE, PASS_EDGE),
        ) == ["b", "c", "x", "y"]
...
def test_component_top_level_and_or_across_kinds():
    ...
    with _served().start() as server:
        ...
        assert _names(
            ...
            "a",
            "outComponent",
            '{or: [{graph: {layers: {names: ["has"]}}}, {edge: {layers: {names: ["owns"], expr: %s}}}]}'
            % PASS_EDGE,
        ) == ["b", "c", "x", "y"]
...
def test_component_external_graph_filter_composed_with_select():
    ...
    with _served().start() as server:
        ...
        q = (
            ...
            '{ node(name: "a") { outComponent(select: {edge: {layers: {names: ["owns"], expr: %s}}}) '
            "{ list { name } } } } } }" % PASS_EDGE
        )
        got = client.query(q)["graph"]["filterNodes"]["node"]["outComponent"]["list"]
