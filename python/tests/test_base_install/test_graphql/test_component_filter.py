...
def test_out_component_scoped_by_edge_layer():
    with _served().start() as server:
        ...
        assert _names(
            client, "a", "outComponent", '{edges: {layers: {names: ["owns"], expr: %s}}}' % PASS_EDGE
        ) == ["b", "c"]
        # scope to `has` -> only a's own satellite (owns edges are not followed)
        assert _names(
            client, "a", "outComponent", '{edges: {layers: {names: ["has"], expr: %s}}}' % PASS_EDGE
        ) == ["x"]
...
def test_component_filter_and_or_combinators():
    ...
    with _served().start() as server:
        ...
        assert _names(
            ...
            "a",
            "outComponent",
            '{edges: {and: [{layers: {names: ["owns"], expr: %s}}, %s]}}' % (PASS_EDGE, PASS_EDGE),
        ) == ["b", "c"]
...
def test_in_component_scoped_by_filters():
    with _served().start() as server:
        ...
        # edge filter
        assert _names(
            client, "c", "inComponent", '{edges: {layers: {names: ["owns"], expr: %s}}}' % PASS_EDGE
        ) == ["a", "b"]
        # graph (layer) filter
