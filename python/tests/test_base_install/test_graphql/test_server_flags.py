...
def test_disable_introspection():
    work_dir = tempfile.mkdtemp()
    with GraphServer(work_dir, config={"schema": {"disable_introspection": True}}).start() as server:
        client = server.get_client()
...
def test_max_query_depth():
    work_dir = tempfile.mkdtemp()
    with GraphServer(work_dir, config={"schema": {"max_query_depth": 3}}).start() as server:
        client = server.get_client()
        make_graph(client)
...
def test_max_query_complexity():
    work_dir = tempfile.mkdtemp()
    with GraphServer(work_dir, config={"schema": {"max_query_complexity": 3}}).start() as server:
        client = server.get_client()
        make_graph(client)
...
def test_disable_lists_all_resolvers():
    ...
    work_dir = tempfile.mkdtemp()
    with GraphServer(work_dir, config={"concurrency": {"disable_lists": True}}).start() as server:
        client = server.get_client()
        make_graph(client)
...
def test_max_page_size_all_resolvers():
    ...
    work_dir = tempfile.mkdtemp()
    with GraphServer(work_dir, config={"concurrency": {"max_page_size": 2}}).start() as server:
        client = server.get_client()
        make_graph(client)
...
def test_disable_batching():
    work_dir = tempfile.mkdtemp()
    with GraphServer(work_dir, config={"concurrency": {"disable_batching": True}}).start() as server:
...
def test_max_batch_size():
    work_dir = tempfile.mkdtemp()
    with GraphServer(work_dir, config={"concurrency": {"max_batch_size": 2}}).start() as server:
        port = server.port()
...
def test_max_recursive_depth():
    work_dir = tempfile.mkdtemp()
    with GraphServer(work_dir, config={"schema": {"max_recursive_depth": 2}}).start() as server:
        client = server.get_client()
        make_graph(client)
...
def test_max_directives_per_field():
    work_dir = tempfile.mkdtemp()
    with GraphServer(work_dir, config={"schema": {"max_directives_per_field": 1}}).start() as server:
        client = server.get_client()
...
def test_concurrency_flags_smoke():
    ...
    with GraphServer(
            work_dir,
            config={"concurrency": {"heavy_query_limit": 4,
