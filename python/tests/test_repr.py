...
class PyReprTest(TestCase):
    ...
    def test_layers_and_props(self):
        ...
        expected_out = "Edge(source=A, target=B, earliest_time=1, latest_time=2, properties={greeting: yo, greeting: yo}, layer(s)=[layer 1, layer 2])\n"
        with patch("sys.stdout", new=StringIO()) as fake_out:
            print(G.edge("A", "B"))
            self.assertEqual(fake_out.getvalue(), expected_out)
    ...
    def layers_and_non_layers(self):
        ...
        expected_out = "Edge(source=A, target=B, earliest_time=1, latest_time=2, properties={greeting: yo, greeting: yo}, layer(s)=[_default, layer 1])\n"
        with patch("sys.stdout", new=StringIO()) as fake_out:
            print(G.edge("A", "B"))
            self.assertEqual(fake_out.getvalue(), expected_out)
    ...
    def test_persistent_graph(self):
        ...
        expected_out = "Edge(source=A, target=B, earliest_time=1, latest_time=6, properties={greeting: yo, greeting: yo}, layer(s)=[layer 1, layer 2])\n"
        with patch("sys.stdout", new=StringIO()) as fake_out:
            print(G.edge("A", "B"))
            self.assertEqual(fake_out.getvalue(), expected_out)
