use raphtory_api::core::utils::errors::{GraphError, IllegalSet};
    ...
    A: PartialEq + Default + Clone + Debug + Send + Sync,
{
    ...
    pub(crate) fn set(&mut self, id: usize, value: A) -> Result<(), IllegalSet> {
        match self {
            LazyVec::Empty => {
            ...
            LazyVec::LazyVec1(only_id, only_value) => {
                if *only_id == id {
                    if *only_value != Default::default() && *only_value != value {
                        return Err(IllegalSet::new(id, only_value, &value));
                    }
                } else {
            ...
            LazyVec::LazyVecN(vector) => {
                ...
                if vector[id] == Default::default() {
                    ...
                    return Err(IllegalSet::new(id, &vector[id], &value));
                }
                Ok(())
            }
        }
