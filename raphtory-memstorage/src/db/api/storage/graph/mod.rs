...
impl GraphStorage {
    ...
    pub fn layer_ids_iter<G: StorageGraphViewOps>(
        &self,
        view: &G,
    ...
    pub fn count_nodes<G: StorageGraphViewOps>(&self, view: &G) -> usize {
    ...
    // pub fn into_nodes_iter<G: StorageGraphViewOps>(
    //     self,
    //     view: G,
    ...
    //             if view.node_list_trusted() {
    //                 Box::new(iter.filter(move |&vid| {
    //                     type_filter[self.node_entry(vid).as_ref().node_type_id()]
    //                 }))
    //             } else {
    //                 Box::new(iter.filter(move |&vid| {
    //                     let node_entry = self.node_entry(vid);
    //                     let node = node_entry.as_ref();
    ...
    pub fn nodes_par<'a, G: StorageGraphViewOps>(
        &'a self,
        view: &'a G,
    ...
    pub fn into_nodes_par<G: StorageGraphViewOps>(
        self,
        view: G,
        type_filter: Option<Arc<[bool]>>,
    ) -> impl ParallelIterator<Item = VID> {
        view.node_list().into_par_iter().filter(move |&vid| {
            let node_entry = self.node_entry(vid);
            let node = node_entry.as_ref();
            let r = type_filter
            ...
            let s = view.filter_node(node, view.layer_ids());
            r && s
        })
    }
    ...
    pub fn edges_iter<'a, G: StorageGraphViewOps + 'a>(
        ...
        let filtered = match view.filter_state() {
            ...
            FilterState::Edges | FilterState::BothIndependent => FilterVariants::Edges(
                iter.filter(move |e| view.filter_edge(e.as_ref(), view.layer_ids())),
            ),
        };
        filtered.map(|e| e.as_ref().out_ref())
    }

    pub fn into_edges_iter<G: StorageGraphViewOps>(
        self,
        view: G,
            ...
            EdgesStorage::Mem(edges) => {
                ...
                let filtered = match view.filter_state() {
                    FilterState::Neither => FilterVariants::Neither(iter.filter_map(move |eid| {
                        ...
                        e.has_layers(view.layer_ids()).then(|| e.out_ref())
                    })),
    ...
    pub fn edges_par<'graph, G: StorageGraphViewOps + 'graph>(
        &'graph self,
        view: G,
        ...
        self.edges()
            ...
            .map(|e| e.as_ref().out_ref())
    }

    pub fn into_edges_par<G: StorageGraphViewOps>(
        self,
        view: G,
    ) -> impl ParallelIterator<Item = EdgeRef> {
        let edges = self.owned_edges();
        let nodes = self.owned_nodes();
    ...
    pub fn node_neighbours_iter<'a, 'graph: 'a, G: StorageGraphViewOps>(
        &'a self,
        node: VID,
        dir: Direction,
        view: &'a G,
    ...
    pub fn into_node_neighbours_iter<G: StorageGraphViewOps>(
        self,
        node: VID,
        dir: Direction,
        view: G,
    ) -> impl Iterator<Item = VID> {
    ...
    #[inline]
    pub fn node_degree<'graph, G: StorageGraphViewOps>(
        &self,
        node: VID,
        dir: Direction,
        view: &G,
        ...
        if matches!(view.filter_state(), FilterState::Neither) {
            self.node_entry(node).as_ref().degree(view.layer_ids(), dir)
        } else {
            self.node_neighbours_iter(node, dir, view).count()
        }
    }

    pub fn node_edges_iter<'a, 'graph: 'a, G: StorageGraphViewOps>(
        &'a self,
        node: VID,
        dir: Direction,
        view: &'a G,
    ...
    pub fn into_node_edges_iter<G: StorageGraphViewOps>(
        self,
        node: VID,
        dir: Direction,
        view: G,
