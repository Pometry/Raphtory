...
#zookeeperLoc = sys.argv[3]
zookeeperLoc = "161.23.245.16:2181,161.23.245.16:2182,161.23.245.16:2183"
...
#seed node
file.write("(docker run -p $SeedPort:2551 --rm -e \"HOST_IP=$IP\" -e \"HOST_PORT=$SeedPort\" $Image seed $IP:$SeedPort $ZooKeeper &) > logs/seed.txt \n")
file.write("sleep 5 \n")
file.write("echo \"Seed node up and running at $IP:$SeedPort\" \n \n")

#Rest API Node
file.write("(docker run -p $RestPort:2551 -p 8080:8080 --rm -e \"HOST_IP=$IP\" -e \"HOST_PORT=$RestPort\" $Image rest $ZooKeeper &) > logs/rest.txt \n")
file.write("sleep 1 \n")
file.write("echo \"REST API node up and running at $IP:$RestPort\" \n \n")

#Update Node
file.write("(docker run -p $UpdatePort:2551  --rm -e \"HOST_IP=$IP\" -e \"HOST_PORT=$UpdatePort\" $Image updateGen $NumberOfPartitions $ZooKeeper &) > logs/updateGenerator.txt \n")
file.write("sleep 1 \n")
file.write("echo \"Update Generator up and running at $IP:$UpdatePort\" \n \n")

#benchmark node
file.write("(docker run -p $BenchmarkPort:2551  --rm -e \"HOST_IP=$IP\" -e \"HOST_PORT=$BenchmarkPort\" $Image benchmark $NumberOfPartitions $ZooKeeper &) > logs/benchmark.txt \n")
file.write("sleep 1 \n")
...
#Routers
for i in range(0,NumberOfRouters):
	file.write("(docker run -p $Router"+str(i)+"Port:2551  --rm -e \"HOST_IP=$IP\" -e \"HOST_PORT=$Router"+str(i)+"Port\" $Image router $NumberOfPartitions $ZooKeeper &) > logs/router"+str(i)+".txt \n")
	file.write("sleep 1 \n")
...
#partition nodes
for i in range(0,NumberOfPartitions):
	file.write("(docker run -p $PM"+str(i)+"Port:2551  --rm -e \"HOST_IP=$IP\" -e \"HOST_PORT=$PM"+str(i)+"Port\" -v $entityLogs:/logs/entityLogs $Image partitionManager $PM"+str(i)+"ID $NumberOfPartitions $ZooKeeper &) > logs/partitionManager"+str(i)+".txt \n")
	file.write("sleep 2 \n")
	file.write("echo \"Partition Manager $PM"+str(i)+"ID up and running at $IP:$PM"+str(i)+"Port\" \n \n")
	
file.write("(docker run -p $LiveAnalysisPort:2551  --rm -e \"HOST_IP=$IP\" -e \"HOST_PORT=$LiveAnalysisPort\" $Image LiveAnalysisManager $NumberOfPartitions $ZooKeeper &) > logs/LiveAnalysisManager.txt \n")
file.write("sleep 1 \n")
file.write("echo \"Live Analyser running at $IP:$LiveAnalysisPort\" \n \n")


file.write("(docker run -p $ClusterUpPort:2551  --rm -e \"HOST_IP=$IP\" -e \"HOST_PORT=$ClusterUpPort\" $Image ClusterUp $NumberOfPartitions $ZooKeeper &) > logs/ClusterUp.txt \n")
file.write("sleep 1 \n")
