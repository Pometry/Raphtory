...
object LargestConnectedComponent extends Generic{
  override def apply(graph: GraphPerspective): graph.Graph =
    ConnectedComponents().apply(graph)
  ...
  override def tabularise(graph: GraphPerspective): Table = graph.globalSelect(
          state => Row(state("components").value.asInstanceOf[Counter[Long]].largest._2)
  )
}
