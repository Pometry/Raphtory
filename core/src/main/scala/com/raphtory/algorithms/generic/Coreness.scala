...
class Coreness(start: Int, end: Int) extends Generic {
  ...
  if (start > end | start < 0) // out of range
    throw new IllegalArgumentException(
            s"start and end parameters must be non-negative and start <= end. You inputted start=$start, end=$end."
    )
  ...
  override def apply(graph: GraphPerspective): graph.Graph = {
    ...
    for (k <- start to end)
      g = KCore(k, resetStates = false).apply(g).clearMessages().step { vertex =>
        if (
                vertex.getState[Int](EFFDEGREE) >= k
        ) // if still in the core graph for this value of k, increment the coreness
          vertex.setState(CORENESS, k)
      }
    g
  }

  override def tabularise(graph: GraphPerspective): Table = graph.select("name", CORENESS)
}
