...
import com.raphtory.api.analysis.table.{Explode, Row}
...
class PythonExplodeEvaluator[IO[_]: Functor](pyObjBytes: Array[Byte], py: EmbeddedPython[IO])
  ...
  override def apply(r: Row): IO[IterableOnce[Row]] =
    py.invoke(PyRef(eval_name), "eval_from_jvm", Vector(new PythonRow(r))).map {
      case arr: Array[Any] => arr.iterate(a => Row(a: _*))
    }
