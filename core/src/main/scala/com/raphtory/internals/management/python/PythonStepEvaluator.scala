...
class PythonStepEvaluator[IO[_]](bytes: Array[Byte], py: EmbeddedPython[IO])
        extends (Vertex => IO[Unit])
        with AutoCloseable {
  private val eval_name = Random.alphanumeric.take(32).mkString
  py.set(s"${eval_name}_bytes", bytes)
