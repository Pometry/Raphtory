...
case class SyncExistingEdgeAddArrowFlightMessageSchema[
  ...
  override def decodeMessage[T](row: Int): T =
    try {
      val msg = getMessageAtRow(row).asInstanceOf[SyncExistingEdgeAddArrowFlightMessage]
  ...
  override def encodeMessage[T](msg: T): ArrowFlightMessage =
    try {
      val vadd                    = msg.asInstanceOf[SyncExistingEdgeAdd]
...
class SyncExistingEdgeAddArrowFlightMessageSchemaFactory extends ArrowFlightMessageSchemaFactory {
