...
case class EdgeAddArrowFlightMessageSchema[
  ...
  override def decodeMessage[T](row: Int): T =
    try {
      val msg        = getMessageAtRow(row).asInstanceOf[EdgeAddArrowFlightMessage]
      val maybeEType = if (msg.eType == "") None else Some(Type(msg.eType))
  ...
  override def encodeMessage[T](msg: T): ArrowFlightMessage =
    try {
      val vadd  = msg.asInstanceOf[EdgeAdd]
      val eType = if (vadd.eType.isDefined) vadd.eType.get.name else ""
...
class EdgeAddArrowFlightMessageSchemaFactory extends ArrowFlightMessageSchemaFactory {

  private def getVectors(vectorSchemaRoot: VectorSchemaRoot): EdgeAddArrowFlightMessageSchema[
    ...
    val eTypes                  = vectorSchemaRoot.getVector("eTypes").asInstanceOf[VarCharVector]
    ...
    val schema: Schema =
      new Schema(
              List(
                      ...
                      new Field("eTypes", new FieldType(false, new ArrowType.Utf8(), null), null),
                      new Field(
                              "immutablePropertyKeys",
