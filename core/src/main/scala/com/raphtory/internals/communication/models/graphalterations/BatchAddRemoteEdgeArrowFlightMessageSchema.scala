...
case class BatchAddRemoteEdgeArrowFlightMessageSchema[
  ...
  override def decodeMessage[T](row: Int): T =
    try {
      val msg           = getMessageAtRow(row).asInstanceOf[BatchAddRemoteEdgeArrowFlightMessage]
  ...
  override def encodeMessage[T](msg: T): ArrowFlightMessage =
    try {
      val vadd  = msg.asInstanceOf[BatchAddRemoteEdge]
...
class BatchAddRemoteEdgeArrowFlightMessageSchemaFactory extends ArrowFlightMessageSchemaFactory {
