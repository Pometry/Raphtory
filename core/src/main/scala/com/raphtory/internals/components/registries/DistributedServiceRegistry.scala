...
import com.raphtory.internals.components.repositories.ServiceDiscovery
...
class DistributedServiceRegistry[F[_]: Async](serviceDiscovery: ServiceDiscovery[F], conf: Config) extends ServiceRegistry[F](conf) {
...
object DistributedServiceRegistry {
  ...
  def apply[F[_]: Async](config: Config): Resource[F, ServiceRegistry[F]] =
    for {
      ...
      serviceRepo <- Resource.eval(Async[F].delay(new DistributedServiceRegistry[F](new ServiceDiscovery[F](client), config)))
    } yield serviceRepo
