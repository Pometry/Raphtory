...
abstract class BaseRaphtoryAlgoTest[T: ClassTag: TypeTag](deleteResultAfterFinish: Boolean = true)
  ...
  beforeAll {
    ...
    graph = batchLoading() match {
      case true => Raphtory.batchLoadGraph[T](spout, graphBuilder)
      case false => Raphtory.streamGraph[T](spout, graphBuilder)
    }

    val conf: Config          = graph.getConfig()
