...
| --tracing-level             | TRACING_LEVEL             | <p>Set the tracing level. Available options are:</p><ul><li><strong>COMPLETE</strong>: Provides full traces for each query.</li><li><strong>ESSENTIAL</strong>: Tracks key functions — addEdge, addEdges, deleteEdge, graph, updateGraph, addNode, node, nodes, edge, edges.</li><li><strong>MINIMAL</strong>: Provides only summary execution times.</li></ul> |
