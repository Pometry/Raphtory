...
The core of the Raphtory algorithm API is the [{s}`GraphAlgorithm`](com.raphtory.core.algorithm.GraphAlgorithm)
...
[{s}`GraphPerspective`](com.raphtory.core.algorithm.GraphPerspective)
class and tabular data by the [{s}`Table`](com.raphtory.core.algorithm.Table) class.
Rows in a [{s}`Table`](com.raphtory.core.algorithm.Table) are manipulated using the
[{s}`Row`](com.raphtory.core.algorithm.Row) class.
...
There are two other classes that form part of the algorithm API, [{s}`Chain`](com.raphtory.core.algorithm.Chain)
and [{s}`Identity`](com.raphtory.core.algorithm.Identity).
A [{s}`Chain`](com.raphtory.core.algorithm.Chain) applies a sequence of algorithms to the same graph and is discussed
in more detail later.
[{s}`Identity`](com.raphtory.core.algorithm.Identity) is an algorithm that leaves the graph unchanged and does
...
```
The {s}`apply()` method takes a [{s}`GraphPerspective`](com.raphtory.core.algorithm.GraphPerspective) as input,
manipulates the state of vertices, and then returns the
[{s}`GraphPerspective`](com.raphtory.core.algorithm.GraphPerspective), either for further processing by
other algorithms or for collecting and writing out results.
A [{s}`GraphPerspective`](com.raphtory.core.algorithm.GraphPerspective) has two key methods which are used during
...
[{s}`Vertex`](com.raphtory.core.graph.visitor.Vertex) class, which exposes methods for storing computational state,
messaging other vertices, accessing edges (represented by the [{s}`Edge`](com.raphtory.core.graph.visitor.Edge) class),
...
using the [{s}`Vertex.voteToHalt()`](com.raphtory.core.graph.visitor.Vertex) method.  All of this can be seen in the example below:
...
method can optionally take a second [{s}`GraphState`](com.raphtory.core.algorithm.GraphState) argument. Before we can
use the global state, we first have to define some accumulators using the {s}`GraphPerspective.setGlobalState()` method.
The [{s}`Accumulator`](com.raphtory.core.algorithm.Accumulator) class has a {s}`+=` operator to add new values to the
...
[{s}`GraphAlgorithm`](com.raphtory.core.algorithm.GraphAlgorithm) class, i.e.
...
`select()` maps a vertex to a [s}`Row`](com.raphtory.core.algorithm.Row) object containing the results for that vertex.
...
step can also optionally take the global [{s}`GraphState`](com.raphtory.core.algorithm.GraphState) as an additional input.
The final method for tabularising results is the {s}`globalSelect()` method which maps the global
[{s}`GraphState`](com.raphtory.core.algorithm.GraphState) to
a single [{s}`Row`](com.raphtory.core.algorithm.Row). For example, we could use this to return the maximum degree
computed above:
...
[{s}`Table`](com.raphtory.core.algorithm.Table) format by the {s}`select()` call.
For example:
...
This is implemented by using an [{s}`OutputFormat`](com.raphtory.core.algorithm.OutputFormat) which is given to the
...
[{s}`Chain`](com.raphtory.core.algorithm.Chain) class.
...
One way to express this in Raphtory is to use a [{s}`Chain`](com.raphtory.core.algorithm.Chain), i.e,
```scala
...
```
The {s}`apply()` method of a [{s}`Chain`](com.raphtory.core.algorithm.Chain) simply calls the {s}`apply()` method
...
Instead of using [{s}`Chain`](com.raphtory.core.algorithm.Chain) directly, one can also use the {s}`->` operator
of [{s}`GraphAlgorithm`](com.raphtory.core.algorithm.GraphAlgorithm) to achieve the same result, i.e.,

```scala
val lpa_cbod = LPA() -> CBOD(label="lpalabel")
```
which constructs the same [{s}`Chain`](com.raphtory.core.algorithm.Chain) object under the hood.
...
[{s}`Identity`](com.raphtory.core.algorithm.Identity) algorithm.
