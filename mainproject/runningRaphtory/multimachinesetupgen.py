...
def partitionManagerRun(files):
	#partition nodes
	for i in range(0,NumberOfPartitions):
		files[(i%NumberOfMachines)].write("(docker run --name=\"partition"+str(i)+"\" -p $PM"+str(i)+"Port:$PM"+str(i)+"Port --rm -e \"BIND_PORT=$PM"+str(i)+"Port\" -e \"HOST_IP=$IP\" -e \"HOST_PORT=$PM"+str(i)+"Port\" -v $logsdir:/logs $Image partitionManager $PM"+str(i)+"ID $NumberOfPartitions $ZooKeeper &) 2>&1 > logs/"+files[(i%NumberOfMachines)].name[:len(files[(i%NumberOfMachines)].name)-3]+"/partitionManager"+str(i)+".txt \n")
...
def routerRun(files):
	#Routers
	for i in range(0,NumberOfRouters):
		files[(i%NumberOfMachines)].write("(docker run --name=\"router"+str(i)+"\" -p $Router"+str(i)+"Port:$Router"+str(i)+"Port  --rm -e \"BIND_PORT=$Router"+str(i)+"Port\" -e \"HOST_IP=$IP\" -e \"HOST_PORT=$Router"+str(i)+"Port\" $Image router $NumberOfPartitions $ZooKeeper &) 2>&1 > logs/"+files[(i%NumberOfMachines)].name[:len(files[(i%NumberOfMachines)].name)-3]+"/router"+str(i)+".txt \n")
...
def updateRun(files):
	#Updaters
	for i in range(0,NumberOfUpdaters):
		files[(i%NumberOfMachines)].write("(docker run --name=\"updater"+str(i)+"\" -p $Update"+str(i)+"Port:$Update"+str(i)+"Port  --rm -e \"BIND_PORT=$Update"+str(i)+"Port\" -e \"HOST_IP=$IP\" -e \"HOST_PORT=$Update"+str(i)+"Port\" $Image updateGen $NumberOfPartitions $ZooKeeper &) 2>&1 > logs/"+files[(i%NumberOfMachines)].name[:len(files[(i%NumberOfMachines)].name)-3]+"/updateGenerator"+str(i)+".txt \n")
		files[(i%NumberOfMachines)].write("sleep 1 \n")
		files[(i%NumberOfMachines)].write("echo \"Update Generator "+str(i)+" up and running at $IP:$Update"+str(i)+"Port\" \n \n")		

def dockerblock(file):
	#seed node
	file.write("(docker run --name=\"seednode\" -p $SeedPort:$SeedPort --rm -e \"BIND_PORT=$SeedPort\" -e \"HOST_IP=$IP\" -e \"HOST_PORT=$SeedPort\" $Image seed $IP:$SeedPort $ZooKeeper &) 2>&1 > logs/"+file.name[:len(file.name)-3]+"/seed.txt \n")
	file.write("sleep 5 \n")
	file.write("echo \"Seed node up and running at $IP:$SeedPort\" \n \n")
	
	#Rest API Node
	file.write("(docker run --name=\"rest\"  -p $RestPort:$RestPort -p 8080:8080 --rm -e \"BIND_PORT=$RestPort\" -e \"HOST_IP=$IP\" -e \"HOST_PORT=$RestPort\" $Image rest $ZooKeeper &) 2>&1 > logs/"+file.name[:len(file.name)-3]+"/rest.txt \n")
	file.write("sleep 1 \n")
...
def complete(file):
	...
	file.write("(docker run --name=\"clusterUp\"  -p $ClusterUpPort:$ClusterUpPort  --rm -e \"BIND_PORT=$ClusterUpPort\" -e \"HOST_IP=$IP\" -e \"HOST_PORT=$ClusterUpPort\" $Image ClusterUp $NumberOfPartitions $ZooKeeper &) 2>&1 > logs/"+file.name[:len(file.name)-3]+"/ClusterUp.txt \n")
	file.write("sleep 15 \n")
