...
class OAGGraphBuilder extends GraphBuilder[String] {
  ...
  def resolveInputFormat(inputstr: String): Int = {
    ...
    if (inputstr.contains("Ti") && inputstr.contains("Ty") && inputstr.contains("Id")
        && inputstr.contains("logprob")) //is MAG format
      return 1
  ...
  private def handleReferences(
      references: List[OpenAcademic],
      commands: ParHashSet[GraphUpdate],
      timestamp: Long,
      annotationUUID: Long,
      outbound: Boolean
  ) =
                ...
                Properties(
                        ...
                        if (reference.labelDensity != None) {
                          ...
                          if (label_density > threshold)
                            ...
                            print(
                                    "skipping as label density:" + label_density + " is not greater than threshold:" + threshold
                            )
                            null
                          }
                        } else
