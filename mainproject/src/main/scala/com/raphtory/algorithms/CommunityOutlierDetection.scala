...
class CommunityOutlierDetection(args: Array[String]) extends LPA(args) {
  //args = [top output, edge property, threshold, maxIter]
  val topnum: Int           = if (arg.length == 0) 0 else arg.head.toInt
  val thr: Double           = if (arg.length < 3) 0.0 else arg(2).toDouble
  override val maxIter: Int = if (arg.length < 4) 500 else arg(3).toInt
  ...
  override def returnResults(): Any =
    view
      ...
      .map(vertex => (vertex.ID(), vertex.getOrSetState[Double]("outlierscore", -1.0))) //TODO: IM: temp fix -- to be remove later
