...
class Reader(
  ...
  for (i <- 0 until workerCount) {
    log.debug("Initialising [{}] worker children for Reader with id [{}}.", workerCount, managerId)
  ...
  override def preStart(): Unit =
    log.debug("Reader actor with id [{}] is being started.", managerId)
  ...
  override def receive: Receive = {
    ...
    case Terminated(child) =>
      log.warning(s"Child worker with path [{}] belonging to Reader id [{}] has died.", child.path, managerId)
    case x => log.warning(s"Reader received unknown message [{}].", x)
  }

  def handleAnalyserPresentCheckRequest(req: AnalyserPresentCheck): Unit = {
    log.debug(s"Reader received [{}] request.", req)
  ...
  def handleCompileNewAnalyserRequest(req: CompileNewAnalyser): Unit = {
    log.debug("Reader received [{}] request.", req)
