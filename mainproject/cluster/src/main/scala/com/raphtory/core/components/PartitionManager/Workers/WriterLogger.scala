...
class WriterLogger extends Actor {
  ...
  override def receive: Receive = {
    case ReportIntake(mainMessages, secondaryMessages, workerMessages, partitionId, timeDifference) =>
      reportIntake(mainMessages, secondaryMessages, workerMessages, partitionId, timeDifference)
...
//  }
  ...
  def reportIntake(
      messageCount: Int,
      secondaryMessageCount: Int,
      workerMessageCount: Int,
      id: Int,
      timeDifference: Long
  ): Unit =
    try
