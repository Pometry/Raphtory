...
case class SingleNodeSetup(seedLoc:String,routerClassName:String,UpdaterName:String,LAMName:String,partitionNumber:Int,minimumRouters:Int) extends DocSvr {
  ...
  sys.env.getOrElse("LAMTYPE", "Live").toString match {
    ...
    case "View" => { //view of the graph
      ...
      sys.env.getOrElse("WINDOWTYPE", "false") match {
        ...
        case "true" => {
          ...
          system.actorOf(Props(new WindowedViewAnalysisManager(jobID, analyser, time, window)), s"ViewAnalysisManager_$LAMName")
        }
        case "batched" => {
          val windowset = sys.env.getOrElse("WINDOWSET", "0").split(",").map(f => f.toLong)
          system.actorOf(Props(new BWindowedViewAnalysisManager(jobID, analyser, time, windowset)), s"ViewAnalysisManager_$LAMName")
        }
      }
    }
    case "Range" => { // windowed range query through history
      ...
      sys.env.getOrElse("WINDOWTYPE", "false") match {
        case "false" => {
          system.actorOf(Props(new RangeAnalysisManager(jobID, analyser, start, end, jump)), s"RangeAnalysisManager_$LAMName")
        }
        case "true" => {
          val window = sys.env.getOrElse("WINDOW", "0").toLong
          system.actorOf(Props(new WindowedRangeAnalysisManager(jobID, analyser, start, end, jump, window)), s"RangeAnalysisManager_$LAMName")
        }
        case "batched" => {
          val windowset = sys.env.getOrElse("WINDOWSET", "0").split(",").map(f => f.toLong)
          system.actorOf(Props(new BWindowedRangeAnalysisManager(jobID, analyser, start, end, jump, windowset)), s"RangeAnalysisManager_$LAMName")
        }
      }
    }
  }
