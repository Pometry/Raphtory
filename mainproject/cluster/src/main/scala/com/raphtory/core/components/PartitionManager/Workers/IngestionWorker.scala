...
class IngestionWorker(workerID:Int) extends Actor {
  ...
  override def receive: Receive = {
    case VertexAdd(routerID, msgTime, srcId) => EntityStorage.vertexAdd(routerID, workerID, msgTime, srcId)
      vHandle(srcId, msgTime)
    case VertexRemoval(routerID, msgTime, srcId) => EntityStorage.vertexRemoval(routerID, workerID, msgTime, srcId)
      vHandle(srcId, msgTime)
    case VertexAddWithProperties(routerID, msgTime, srcId, properties) => EntityStorage.vertexAdd(routerID, workerID, msgTime, srcId, properties)
      vHandle(srcId, msgTime)

    case DstAddForOtherWorker(routerID, msgTime, dstID, srcForEdge, edge, present) => EntityStorage.vertexWorkerRequest(routerID, workerID, msgTime, dstID, srcForEdge, edge, present)
      wHandle()
    case DstWipeForOtherWorker(routerID, msgTime, dstID, srcForEdge, edge, present) => EntityStorage.vertexWipeWorkerRequest(routerID, workerID, msgTime, dstID, srcForEdge, edge, present)
      wHandle()
    case DstResponseFromOtherWorker(routerID, msgTime, srcForEdge, dstID, removeList) => EntityStorage.vertexWorkerRequestEdgeHandler(routerID, workerID, msgTime, srcForEdge, dstID, removeList)
      wHandle()
    case EdgeRemoveForOtherWorker(routerID, msgTime, srcID, dstID) => EntityStorage.edgeRemovalFromOtherWorker(routerID, workerID, msgTime, srcID, dstID)
      wHandle()
    ...
    case EdgeAdd(routerID, msgTime, srcId, dstId) => EntityStorage.edgeAdd(routerID, workerID, msgTime, srcId, dstId)
      eHandle(srcId, dstId, msgTime)
    case EdgeAddWithProperties(routerID, msgTime, srcId, dstId, properties) => EntityStorage.edgeAdd(routerID, workerID, msgTime, srcId, dstId, properties)
      eHandle(srcId, dstId, msgTime)

    case RemoteEdgeAdd(routerID, msgTime, srcId, dstId, properties) => EntityStorage.remoteEdgeAdd(routerID, workerID, msgTime, srcId, dstId, properties)
      eHandleSecondary(srcId, dstId, msgTime)
    case RemoteEdgeAddNew(routerID, msgTime, srcId, dstId, properties, deaths) => EntityStorage.remoteEdgeAddNew(routerID, workerID, msgTime, srcId, dstId, properties, deaths)
      eHandleSecondary(srcId, dstId, msgTime)

    case EdgeRemoval(routerID, msgTime, srcId, dstId) => EntityStorage.edgeRemoval(routerID, workerID, msgTime, srcId, dstId)
      eHandle(srcId, dstId, msgTime)
    case RemoteEdgeRemoval(routerID, msgTime, srcId, dstId) => EntityStorage.remoteEdgeRemoval(routerID, workerID, msgTime, srcId, dstId)
      eHandleSecondary(srcId, dstId, msgTime)
    case RemoteEdgeRemovalNew(routerID, msgTime, srcId, dstId, deaths) => EntityStorage.remoteEdgeRemovalNew(routerID, workerID, msgTime, srcId, dstId, deaths)
      eHandleSecondary(srcId, dstId, msgTime)

    case ReturnEdgeRemoval(routerID, msgTime, srcId, dstId) => EntityStorage.returnEdgeRemoval(routerID, workerID, msgTime, srcId, dstId)
      eHandleSecondary(srcId, dstId, msgTime)
    case RemoteReturnDeaths(routerID, msgTime, srcId, dstId, deaths) => EntityStorage.remoteReturnDeaths(routerID, workerID, msgTime, srcId, dstId, deaths)
      eHandleSecondary(srcId, dstId, msgTime)
    ...
    EntityStorage.vertices(workerID).get(key) match {
      case Some(vertex) => {
        ...
        if (vertex.archive(archiveTime, compressing, true, workerID)) {
          EntityStorage.vertices(workerID) //TODO nice way to delete when reenabled
