...
class Writer(
  ...
  override def receive: Receive = {
    ...
    case UpdatedCounter(newValue) =>
      managerCount = newValue; storage.foreach(s => s._2.setManagerCount(managerCount))
    case "keep_alive" =>
      mediator ! DistributedPubSubMediator.Send("/user/WatchDog", PartitionUp(managerID), localAffinity = false)
