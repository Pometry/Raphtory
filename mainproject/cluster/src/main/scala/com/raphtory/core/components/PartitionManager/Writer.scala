...
class Writer(
  ...
  override def preStart(): Unit = {
    log.debug("Writer actor with id [{}] is being started.", managerId)
  ...
  override def receive: Receive = {
    ...
    case Terminated(child) =>
      log.warning(s"Child worker with path [{}] belonging to Writer id [{}] has died.", child.path, managerId)
    case x => log.warning(s"Writer received unknown message [{}].", x)
  }

  def processLogMessage(req: String): Unit = {
    log.debug(s"Writer received [{}] message.", req)

    logChildForSize ! ReportSize(managerId)
  }

  def processCountMessage(msg: String): Unit = {
    log.debug(s"Writer received [{}] message.", msg)
  ...
  def handleUpdatedCounterRequest(req: UpdatedCounter): Unit = {
    log.debug(s"Writer received request [{}]", req)
  ...
  private def scheduleTasks(): Unit = {
    log.debug("Preparing to schedule tasks in Writer.")
