    ...
    val tosend = view
      ...
      .map { vertex =>
        if (vertex.containsState("exchangeHit"))
          ...
          (vertex.getPropertyValue("id").get.asInstanceOf[String], vertex.getState[Long]("infected"),vertex.getState[String]("infectedBy"),false,0)
        else
          ("", -1L,"",false,-1)

      }
      .filter(f => f._2 >= 0)
    println(s"worker $workerID finished")
    tosend
  }
