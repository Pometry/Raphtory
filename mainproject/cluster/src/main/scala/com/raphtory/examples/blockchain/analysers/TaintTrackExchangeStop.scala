...
class TaintTrackExchangeStop(args:Array[String]) extends Analyser(args) {
  ...
  override def setup(): Unit = view.getVertices().foreach { vertex =>
      ...
      if(walletID contains infectedNode) {
        ...
        vertex.getOutEdgesAfter(infectionStartingBlock).foreach { neighbour =>
          neighbour.send((walletID,neighbour.firstActivityAfter(infectionStartingBlock),vertex.ID()))
        }
      }
    }
  ...
  override def analyse(): Unit =
    view.getMessagedVertices().foreach { vertex =>
      ...
      val tuple = queue.filter(x=>x._2==infectionBlock).head //todo check if multiple
      infector = tuple._1
      ...
      if(vertex.containsState("infected")){
        if(infectionBlock>=vertex.getState[Long]("infected")){
          if(listOfExchanges contains walletID ){
            ...
            println(vertex.getInEdge(tuple._3).get.getPropertyValuesAfter("value",1).get.map(x=>x.asInstanceOf[Double]).mkString(","))
            vertex.setState("sent",vertex.getInEdge(tuple._3).get.getPropertyValuesAfter("value",infectionBlock).get.map(x=>x.asInstanceOf[Double]).sum)
            ...
            vertex.getOutEdgesAfter(infectionBlock).foreach { neighbour =>
              neighbour.send((walletID,neighbour.firstActivityAfter(infectionBlock),vertex.ID()))
            }
          }
        }
      }
    ...
    view
      ...
      .map { vertex =>
        if (vertex.containsState("exchangeHit"))
          (vertex.getPropertyValue("id").get.asInstanceOf[String], vertex.getState[Long]("infected"),vertex.getState[String]("infectedBy"),true,vertex.getState[Double]("sent"))
        else if (vertex.containsState("infected"))
          (vertex.getPropertyValue("id").get.asInstanceOf[String], vertex.getState[Long]("infected"),vertex.getState[String]("infectedBy"),false,-1)
        else
          ("", -1L,"",false,-1)
  ...
  override def processResults(results: ArrayBuffer[Any], timeStamp: Long, viewCompleteTime: Long): Unit = {
    val endResults = results.asInstanceOf[ArrayBuffer[immutable.ParIterable[(String, Long,String,Boolean,Double)]]].flatten
