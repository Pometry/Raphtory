...
class EthereumTransactionRouter(val routerId:Int, val initialManagerCount:Int) extends RouterWorker{
  ...
  override protected def parseTuple(value: Any): Unit = {
    ...
    sendGraphUpdate(VertexAddWithProperties(creationDate, sourceNode,Properties(StringProperty("id",components(0)))))//create the source node and add the wallet ID as a property
    if(components(1).nonEmpty) { //money being sent to an actual user
      ...
      sendGraphUpdate(VertexAddWithProperties(creationDate, targetNode,Properties(StringProperty("id",components(1))))) //create the destination vertex
      sendGraphUpdate(EdgeAddWithProperties( creationDate, sourceNode, targetNode,Properties(StringProperty("id",components(2))))) //create the edge between them adding the value as a property
    }
    else{ //burnt cash
      ...
      sendGraphUpdate(EdgeAddWithProperties(creationDate, sourceNode, targetNode,Properties(StringProperty("value",components(2)))))
    }
