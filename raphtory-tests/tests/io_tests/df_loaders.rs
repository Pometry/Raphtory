...
mod parquet_tests {
    ...
    fn check_parquet_encoding(g: &Graph, fixture: Option<GraphFixture>) {
        ...
        let g2 = Graph::decode_parquet(&temp_dir, None, ConfigArgs::default().into()).unwrap();
    ...
    fn check_parquet_encoding_deletions(g: PersistentGraph) {
        let temp_dir = tempfile::tempdir().unwrap();
        g.encode_parquet(&temp_dir).unwrap();
        let g2 = PersistentGraph::decode_parquet(&temp_dir, None, ConfigArgs::default().into()).unwrap();
        assert_graph_equal_timestamps(&g, &g2);
    }
    ...
    fn test_parquet_zip_simple() {
        ...
        let g2 = Graph::decode_parquet_from_zip(
            &mut reader,
            None::<&std::path::Path>,
            "graph",
            ConfigArgs::default().into(),
        )
        .unwrap();
        assert_graph_equal(&g, &g2);
    }
    ...
    fn test_parquet_bytes_simple() {
        ...
        g.encode_parquet_to_zip(&mut writer, "graph").unwrap();
        writer.finish().unwrap();
        let g2 = Graph::decode_parquet_from_bytes(
            &bytes,
            None::<&std::path::Path>,
            "graph",
            ConfigArgs::default().into(),
    ...
    fn test_parquet_bytes_proptest() {
        proptest!(|(edges in build_graph_strat(30, 30, 10, 10, true))| {
            ...
            g.encode_parquet_to_zip(&mut writer, "graph").unwrap();
            writer.finish().unwrap();
            let g2 = Graph::decode_parquet_from_bytes(&bytes, None::<&std::path::Path>, "graph", ConfigArgs::default().into()).unwrap();
