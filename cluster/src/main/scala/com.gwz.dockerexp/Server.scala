...
object Go extends App {
    ...
    case "LiveAnalysisManager" => {
      ...
      LiveAnalysisNode(getConf(args(2)),args(1))
    }
    case "ClusterUp" => {
  ...
  def setConf(seedLoc:String,zookeeper:String):Unit ={
    ...
    val curatorZookeeperClient = CuratorFrameworkFactory.newClient(zookeeper, retryPolicy)
    ...
    if(curatorZookeeperClient.checkExists().forPath("/seednode")==null){
      curatorZookeeperClient.create().creatingParentsIfNeeded().forPath("/seednode", seedLoc.getBytes)
    }
    else{
      curatorZookeeperClient.setData().forPath("/seednode",seedLoc.getBytes)
    }
    val originalData = new String(curatorZookeeperClient.getData.forPath("/seednode"))
