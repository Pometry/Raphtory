...
class MaxFlowTest[T](
  ...
  override def tabularise(graph: GraphPerspective): Table =
    graph.explodeSelect(vertex =>
      if (vertex.name() == source)
        List(Row(KeyPair("", vertex.getState[mutable.Map[Long, T]]("flow").values.sum)))
      else List.empty[Row]
    )
...
object MaxFlowTest {
  ...
  def apply[T](
      ...
      capacityLabel: String = "weight",
      maxIterations: Int = Int.MaxValue
  )(implicit numeric: Numeric[T]): MaxFlowTest[T] =
    new MaxFlowTest(source, target, capacityLabel, maxIterations)(numeric)
}
