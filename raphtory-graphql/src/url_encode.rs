...
pub fn url_decode_graph<T: AsRef<[u8]>>(
    ...
    MaterializedGraph::decode_from_bytes_with_config(&bytes, args)
}
...
pub fn url_decode_graph_at<T: AsRef<[u8]>>(
    ...
    if Extension::disk_storage_enabled() {
        MaterializedGraph::decode_from_bytes_at(&bytes, storage_path, args)
    } else {
        MaterializedGraph::decode_from_bytes_with_config(&bytes, args)
    }
}
...
mod tests {
    ...
    fn test_url_encode_decode() {
        ...
        let decoded_graph = url_decode_graph_at(bytes, &storage_path, Args::default()).unwrap();
