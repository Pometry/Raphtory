...
impl GraphServer {
    ...
    pub async fn new(
        ...
        graph_config_args: ConfigArgs,
    ) -> IoResult<Self> {
        if !work_dir.exists() {
            create_dir_all(&work_dir)?;
        }
        let config = app_config.unwrap_or_default();
        let data = Data::new(work_dir.as_path(), &config, graph_config_args.into_config());
        Ok(Self {
            work_dir,
            data,
            config,
...
mod server_tests {
    ...
    async fn test_batch_size_limit_enforced() {
        ...
        let server = GraphServer::new(work_dir.path().to_path_buf(), None, ConfigArgs::default())
            .await
            .unwrap();
        let running = server.start_with_port(0).await.unwrap();
        let port = running.port();
    ...
    async fn test_server_start_stop() {
        ...
        let server = GraphServer::new(tmp_dir.path().to_path_buf(), None, ConfigArgs::default())
            .await
            .unwrap();
