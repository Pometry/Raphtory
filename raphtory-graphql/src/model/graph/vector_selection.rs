...
// Feature-off stub: present in the schema for shape parity, uninhabited and
// never constructed (see `GqlVectorisedGraph`), so the resolvers are dead.
#[cfg(not(feature = "vectors"))]
#[derive(ResolvedObject)]
#[graphql(name = "VectorSelection")]
pub(crate) struct GqlVectorSelection(#[allow(dead_code)] std::convert::Infallible);
