...
mod graphql_test {
    ...
    async fn test_algorithm_temporal_seir_seed_variants() {
        ...
        let res = setup.schema.execute(Request::new(query)).await;
        assert_eq!(res.errors, vec![], "{:?}", res.errors);
        // With no onward infection only the seeds appear. `number` samples that many
        // nodes; `probability` currently seeds *every* node regardless of the value,
        // because the core `IntoSeeds for Probability` impl ignores it.
        assert_eq!(
            res.data.into_json().unwrap(),
            json!({
                "graph": { "algorithm": {
                    "byNumber": { "count": 2 },
                    "byProbability": { "count": 4 }
                } }
            })
        );
    }
