...
impl RaphtoryGraphQLClient {
    ...
    pub fn connect(url: String, token: Option<String>) -> Result<Self, ClientError> {
        ...
        if response.status() != 200 {
            ...
            return Err(ClientError::HttpStatus(
                status,
                format!(
                    "Could not connect to the given server. Status code {} - response: {}",
                    status, text
                ),
            ));
        }
    ...
    pub async fn try_graphql_query_async<Q: GraphQLQuery>(
        &self,
        vars: Q::Variables,
    ) -> Result<Q::ResponseData, ClientError> {
        let query_body = Q::build_query(vars);
        let response = self
            .client
            .post(&self.url)
        ...
        let resp: Response<Q::ResponseData> = response.json().await?;
        if let Some(e) = resp.errors.as_ref() {
            if !e.is_empty() {
                let message = e
                    .iter()
                    .map(|e| format!("{}", e))
    ...
    pub async fn send_graph_async(
        ...
        let data = self.json_query_async(&query, variables).await?;
