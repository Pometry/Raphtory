...
| Collection (`Nodes`/`Edges`/`PathFromNode`) view ops + `applyViews` | ⬜ `ViewKind` is shared, but `view_nodes`/`view_edges`/`view_path` + `applyViews` folding not wired yet |
...
| Filter gaps via serde bridge | ⬜ **enum-bearing** variants don't deserialize (GraphQL `SCREAMING_SNAKE` enums vs serde `camelCase` rename): node-field filters (`{node:{field:NODE_NAME,…}}`), degree filters, and **time-scoped** filter exprs (`{window/at/…}` — `GqlTimeInput` newtype serde). These currently return `PlanError::Filter`. Fix later by aligning serde renames or hand-parsing the enum/time bits. |
