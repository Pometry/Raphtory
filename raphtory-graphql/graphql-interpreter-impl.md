...
| `Value::EventTime` carries `GqlEventTime` (pattern 3) | ✅ holds `Option<EventTime>`; `timestamp`/`eventId`/`datetime` leaves are null-aware; `history.list` items wrap `Some(..)` (parity preserved). Unblocks nullable time-field navs (`earliestTime`/`latestTime`/`start`/`end`/`firstUpdate`/`lastUpdate`/edge `time`). |
