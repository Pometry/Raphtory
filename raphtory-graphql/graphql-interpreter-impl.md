...
| `Value::EventTime` carries `GqlEventTime` (pattern 3) | ✅ holds `Option<EventTime>`; `timestamp`/`eventId`/`datetime` leaves are null-aware; `history.list` items wrap `Some(..)` (parity preserved). |
| **Time fields** — `earliestTime`/`latestTime`/`start`/`end` (Graph/Node/Edge), `firstUpdate`/`lastUpdate` (Node/Edge) | ✅ navs → `Value::EventTime`; null path verified (`start` with no window → `null`); differential-tested |
| **Scalars** — Graph `countNodes`/`countEdges`/`countTemporalEdges`/`uniqueLayers`/`hasNode`/`hasEdge(layer?)`; Node `nodeType`/`degree`/`inDegree`/`outDegree`/`edgeHistoryCount`/`isActive`; Edge `isActive`/`isValid`/`isDeleted`/`isSelfLoop`/`layerNames` | ✅ Int/Bool/String(+nullable)/String-array leaves; `hasNode`/`hasEdge` carry node-id (+optional layer) args; differential-tested |
| **Traversal** — Node `edges`/`inEdges`/`outEdges` (→Edges), `inNeighbours`/`outNeighbours` (→Path), `inComponent`/`outComponent` (→Nodes); Edge `nbr` (→Node), `explode`/`explodeLayers` (→Edges), `deletions` (→History) | ✅ navs reuse existing `Value` variants; `select:` filter args rejected (`reject_select`) pending filter support; differential-tested |
| Aliases (`foo: field(...)`) | ✅ planner uses `response_key()`; verified by aliased `hasEdge` in surface test |
| **Collection ops** — `count`/`page(limit,offset,pageIndex)` (Nodes/Edges/PathFromNode), `sorted(sortBys:)` (Nodes/Edges) | ✅ `Count` leaf; `*Page(Page)` iters (shared `emit_items` helper, `.skip(start).take(limit)`); `sorted` reuses the model's sort via crate-visible `sorted_view`; `sortBys` parsed by hand at plan time; differential-tested (id/property/time sort, reverse, paging, neighbours count/page) |
| **Graph/Node/Edge views (pattern 1)** — `window`/`at`/`before`/`after`/`latest`/`snapshotAt`/`snapshotLatest`/`shrink*`/`defaultLayer`/`layer`/`layers`/`excludeLayer(s)`; graph-only `valid`/`subgraph`/`subgraphNodeTypes`/`excludeNodes` | ✅ unified `Nav::View(ViewKind)`; old `Window`/`After`/`Before`/`Layer` folded in; exec dispatches per type via `view_graph`/`view_node`/`view_edge`; planner gates which `ViewKind` per receiver (graph-only ops stay graph-only); differential-tested (time/layer/structural/chained views on graph, node, edge) |
| Collection view ops — `Nodes`/`Edges`/`PathFromNode` `window`/`at`/`before`/`after`/`latest`/`snapshot*`/`shrink*`/`layer(s)`/`excludeLayer(s)`/`defaultLayer`; `typeFilter` (Nodes/Path) | ✅ `view_nodes`/`view_edges`/`view_path` extend `Nav::View(ViewKind)` to collections; planner gates via `is_viewable` (SDL validation confirms field existence); `ViewKind::TypeFilter` added; differential-tested (incl. chained collection views) |
| `applyViews` (Graph/Node/Edge/Nodes/Edges/PathFromNode) | ✅ `Nav::ApplyViews(Vec<ViewStep>)` folds a parsed `[*ViewCollection]` list left→right (same type in→out). Each `@oneOf` entry → `ViewStep::{View(ViewKind), NodeFilter(Composite), EdgeFilter(Composite)}` at plan time; `false` boolean flags contribute no step. Reuses `view_*` helpers + `filter`/`filter_view` (added `GqlEdge::filter_view`). Gated by `is_viewable`. Differential-tested (graph window+layers+nodeFilter / edgeFilter / no-op flag; nodes typeFilter+nodeFilter; edges layers+edgeFilter; node window; path layers). |
| **Filtering (pattern 2)** — `graph.filterNodes`/`filterEdges`, `nodes`/`edges`/`neighbours`/`in*`/`out*`(`select:`), `nodes`/`edges`/`path`/`node` `.filter`/`.select` | ✅ parsed at plan time via **serde bridge** (`ConstValue → serde_json → GqlNodeFilter/GqlEdgeFilter → TryInto<Composite*Filter>`), then pushed into raphtory's `filter`/`select`. Sticky `filter` vs one-hop `select` distinguished. Filter parse/convert errors are clean pre-stream `PlanError::Filter`. Differential-tested (property `ge`/`lt`/`gt`/`eq`, And/Or/Not, graph/collection/traversal/node receivers). |
...
| **Temporal aggregates (`TemporalProperty`)** — `values`, `at(t)`, `latest`, `unique`, `orderedDedupe(latestTime:)`, `sum`, `mean`, `average`, `min`, `max`, `median`, `count` | ✅ scalar-list / nullable-scalar leaves over `GqlTemporalProperty` sync helpers; `min`/`max`/`median` → `Value::PropertyTuple` (nullable Nav), `orderedDedupe` → list of `PropertyTuple`; `PropertyTuple.{time,value,asString}` wired; `temporal.get(key:)` (and `properties`/`metadata.get`) added; `time_arg` now uses the real `GqlTimeInput` scalar parser (int/string/`{timestamp,eventId}` for **all** time args). Differential-tested (`unique` excluded — order unspecified). |
| **Properties/Metadata extras** — `keys`, `contains(key:)`; `TemporalProperties.{keys,contains}` | `get(key:)` ✅ done (above); `keys` → array leaf; `contains` → `Bool` leaf (not yet wired). |
...
- Mutations, subscriptions, directives (`@skip`/`@include`), introspection.
  (Fragments and variables are now **supported** — see the status table.)
