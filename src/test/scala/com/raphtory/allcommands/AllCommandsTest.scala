...
//  test("hashtest") {
//
//    val dir1 = new File(testDir+s"/GraphState_1635208286549").listFiles.filter(_.isFile)
//    val results1 = (for(i <- dir1) yield scala.io.Source.fromFile(i).getLines().toList).flatten.sorted(sortOrdering)
//
//    val dir2 = new File(testDir+s"/GraphState_1635208056275").listFiles.filter(_.isFile)
//    val results2 = (for(i <- dir2) yield scala.io.Source.fromFile(i).getLines().toList).flatten.sorted(sortOrdering)
//
//    println(s"${results1.size} ${results2.size}")
//
//
//    //      for (i <- 0 until results1.size) {
