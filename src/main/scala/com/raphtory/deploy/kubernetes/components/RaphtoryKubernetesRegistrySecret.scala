...
import com.raphtory.deploy.kubernetes.components.KubernetesClient

object RaphtoryKubernetesRegistrySecret extends KubernetesClient {

  def create(): Unit =
    if (conf.hasPath("raphtory.deploy.kubernetes.secrets.registry.create") &&
  ...
  def delete(): Unit =
    if (conf.hasPath("raphtory.deploy.kubernetes.secrets.registry.delete") &&
