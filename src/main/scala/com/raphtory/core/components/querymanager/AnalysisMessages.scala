...
import com.raphtory.core.algorithm.{GraphAlgorithm, GraphState, OutputFormat}

trait QueryManagement extends Serializable

case class WatermarkTime(partitionID: Int, time: Long, safe: Boolean)
...
case class CreatePerspective(timestamp: Long, window: Option[Long])
    extends QueryManagement
case class PerspectiveEstablished(vertices: Int) extends QueryManagement
case class SetGraphState(graphState: Map[String, Any])
...
case class GraphFunctionComplete(receivedMessages: Int,
                                 sentMessages: Int,
                                 votedToHalt: Boolean = false,
                                 graphState: Option[GraphState] = None)
    extends QueryManagement
...
case class VertexMessage[+T](superstep: Int, vertexId: Long, data: T)
    extends QueryManagement
case class VertexMessageBatch[T](data: Array[VertexMessage[T]])
    extends QueryManagement

sealed trait Query extends QueryManagement
case class PointQuery(name: String,
                      algorithm: GraphAlgorithm,
                      timestamp: Long,
                      windows: List[Long] = List(),
                      outputFormat: OutputFormat)
    extends Query
case class RangeQuery(name: String,
                      algorithm: GraphAlgorithm,
                      start: Long,
                      end: Long,
                      increment: Long,
                      ...
                      outputFormat: OutputFormat)
    extends Query
case class LiveQuery(name: String,
                     algorithm: GraphAlgorithm,
                     increment: Long,
                     windows: List[Long] = List(),
                     outputFormat: OutputFormat)
