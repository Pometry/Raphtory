  ...
  override def handleMessage(msg: Message[Array[Byte]]): Unit = {
    deserialise[QueryManagement](msg.getValue) match {
      ...
      case VertexMessageBatch(msgBatch) =>
        logger.trace(
                s"Job '$jobID' at Partition '$partitionID': Executing 'VertexMessageBatch', '[${msgBatch.mkString(",")}]'."
        )
        ...
        graphLens
          ...
          .foreach(row =>
            outputFormat match {
              ...
              case format =>
                format.write(graphLens.getTimestamp(), graphLens.getWindow(), jobID, row, partitionID)
