...
final case class StepWithGraph(f: (Vertex, GraphState) => Unit) extends GraphFunction
final case class Iterate(f:(Vertex)=>Unit, iterations:Int,executeMessagedOnly:Boolean) extends GraphFunction
final case class IterateWithGraph(f: (Vertex, GraphState) => Unit, iterations: Int, executeMessagedOnly: Boolean) extends GraphFunction
final case class VertexFilter(f:(Vertex)=>Boolean) extends GraphFunction
final case class VertexFilterWithGraph(f: (Vertex, GraphState) => Boolean) extends GraphFunction
final case class Select(f:Vertex=>Row) extends GraphFunction
final case class SelectWithGraph(f: (Vertex, GraphState) => Row) extends GraphFunction
final case class ClearChain() extends GraphFunction

abstract class GraphPerspective{
  def setup(f: GraphState => Unit): GraphPerspective
  ...
  def iterate(f: (Vertex, GraphState) => Unit, iterations: Int, executeMessagedOnly: Boolean): GraphPerspective
