...
class ChroniclePartition(partition: Int) extends GraphPartition(partition: Int){
  /**
  ...
  def addProperties(msgTime: Long, entity: ChronicleEntity, properties: Properties): Unit =
  ...
  def addVertexInternal(msgTime: Long, srcId: Long, properties: Properties, vertexType: Option[Type]): ChronicleVertex = { //Vertex add handler function
    val vertex: ChronicleVertex = vertices.get(srcId) match { //check if the vertex exists
      case Some(v) => //if it does
        v revive msgTime //add the history point
        v
      case None => //if it does not exist
        val v = new ChronicleVertex(msgTime, srcId, initialValue = true) //create a new vertex
  ...
  def getVertexOrPlaceholder(msgTime: Long, id: Long): ChronicleVertex =
    vertices.get(id) match {
      case Some(vertex) => vertex
      case None =>
        val vertex = new ChronicleVertex(msgTime, id, initialValue = true)
  ...
  def removeVertex(msgTime: Long, srcId: Long, channelId: String, channelTime: Int): List[TrackedGraphEffect[GraphUpdateEffect]] = {
    val vertex = vertices.get(srcId) match {
      ...
      case None => //if the removal has arrived before the creation
        val v = new ChronicleVertex(msgTime, srcId, initialValue = false) //create a placeholder
    ...
    val (present, edge) = srcVertex.getOutgoingEdge(dstId) match {
      ...
      case None => //if it does not
        val newEdge = if (local) {
          new ChronicleEdge(msgTime, srcId, dstId, initialValue = true) //create the new edge, local or remote
        } else {
  ...
  def removeEdge(msgTime: Long, srcId: Long, dstId: Long, channelId: String, channelTime: Int): Option[TrackedGraphEffect[GraphUpdateEffect]] = {
    ...
    val srcVertex: ChronicleVertex = getVertexOrPlaceholder(msgTime, srcId)
    ...
    val (present, edge) = srcVertex.getOutgoingEdge(dstId) match {
      ...
      case None =>
        val newEdge = if (local) {
          new ChronicleEdge(msgTime, srcId, dstId, initialValue = false)
        } else {
