...
import com.raphtory.ethereumtest.EthereumGraphBuilder
...
object Runner extends RaphtoryService[String] {
  val download = "https://raw.githubusercontent.com/Raphtory/Data/main/snap-twitter.csv"

  val status: Int = s"curl -o /tmp/twitter.csv $download" !
  ...
  override def defineSpout(): Spout[String] = StaticGraphSpout(s"/tmp/twitter.csv")

  override def defineBuilder: GraphBuilder[String] = new TwitterGraphBuilder()

  override def defineSchema(): Schema[String] =
    Schema.STRING
}

object RunnerHelper extends App {
