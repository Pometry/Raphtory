...
import com.raphtory.core.algorithm.{GraphPerspective, Row, Table}
...
Parameters
  ...
  proportionCoop (Float) : proportion of vertices (randomly chosen) that start out as cooperators. Alternatively, vertices
    starting with a status "cooperator" set to 0 begin as cooperators.
  benefit (Float) : benefit parameter for prisoners' dilemma game. See [1]
  cost (Float) : cost parameter for prisoners' dilemma game. See [1]
  noGames (Int) : maximum number of games to be played.
Returns
  name (String) : name of the vertex
  cooperation status (List of 0,1) : choice of cooperation status over time (0 = cooperator, 1 = defector)
References
  [1] Nowak, M. A., & May, R. M. (1992). Evolutionary games and spatial chaos. Nature, 359(6398), 826-829.
  **/

class PrisonersDilemma(proportionCoop:Float=0.5f,
                       benefit: Float, cost:Float=1.0f, noGames:Int=100)
extends NodeList(Seq("cooperationHistory")){
  ...
  override def apply(graph: GraphPerspective) : GraphPerspective = {
    ...
    }, executeMessagedOnly = true, iterations = noGames)
  }
  ...
  def playStep(vertex: Vertex, benefit: Float, cost: Float) : Unit = {
    ...
    val payoff = neighbourStatuses.map(neighStatus => (myStatus, neighStatus)).map {
...
object PrisonersDilemma {
  def apply(output:String= "/tmp/prisonerDilemma", proportionCoop:Float=0.5f,
            benefit: Float, cost: Float, noGames:Int=100) =
