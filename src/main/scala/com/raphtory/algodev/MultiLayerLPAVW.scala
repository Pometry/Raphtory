...
import scala.collection.parallel.{ParIterable, ParMap, immutable}
  ...
  override def analyse(): Unit = {
    ...
    try
      view.getMessagedVertices().foreach { vertex =>
        ...
        var voteCount = 0
        val newLabel = vlabel.map { tv => //(ts, array(l1,l2)), size(array)=w, size(vlabel) = w*ts
          val ts = tv._1
      ...
      groupedW.foreach { g =>
        ...
        output_file match {
          ...
          case _ =>
            ...
            val writer =
              ParquetWriter.writer[Data](output_file+s"omega-$w.parquet", ParquetWriter.Options(writeMode = ParquetFileWriter.Mode.OVERWRITE))
